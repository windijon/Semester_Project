...
In order to find the most active, laziest or most adventurous individual we derived the speed variable in order to find a proxy of the most active or laziest wild boar. Further we thought about another proxy of activeness: The net displacement to find the most adventurous individual. In this case to decided to derive the mean speed and mean displacement per one day intervals. In order to accomplish this task:

1. We roundet the date by days
2. We grouped by TierName and datetime_round
3. We derived the mean speed and displacement by calculating mean speed, steplenght and net displacement
4. Plotting with geom_tile , Y = TierName , X = mean hours of the day , fill = mean speed / steplenght / net displacement
...
The idea behind the “Moving Window Approach” is to reduce the noise (fluctuation) of the data. It therefore might be another approach to analyze i.e. the speed_mean variable but was not been further elaborated in this project. The principal idea therefore is to sum up (in this case six and five) variables like speed_mean to smooth out movement peeks over the given time period with the rollmean function from the zoo package. This allows to visualize the movement patterns in a more trend driven way. Another idea would have been to derive the rollmean of the individuals and compare the overall movement trends among each individual. This was not further been elaborated because of the following more aesthetic and better working approaches.
...
Wildboar_brave <- mutate(Wildboar_brave, Human_Influence_Areas = if_else(objektart_Beschreibung == "Wald&Wiesen&Natur", "Natural Habitats", "Human_Influence_Areas"))
...
### The most active / laziest and most adventurous wild boar

The following are the core result of the question answered: which is the most active / laziest  and the most adventurous wild boar. The interpretation is still a vage idea of what "active" / "laziest" means but in this project we defined it as the most active wildboar over a certain time period. The results show clear "winners" and "loosers" when having a look on the mean_speed, mean_steplength and mean_net_displacement over the evaluated timeperiod. We interpreted the results in the way that in our point of view the wildboar with the highest steplength, speed or net displacement has been evaluated as the most active wild boar and vice versa. In order to backup these findings we also derived the overall mean speed and net displacement of each individual to back up our results and therefore finding the overall most active / laziest  and the most adventurous wild boar.
...
We also tried different visualization approaches in order to find the most useful ones to derive the overall "laziness" or "activeness".
...
These boxplots show the overall mean proxies of our indicator like speed /  steplength and net displacement of the individuals and are therefore are clearly the most adequate way to visualize the overall activeness.The overall winner therefore seems to be the individual Fritz whereas he has the highest mean speed, steplength and net displacement over the hole timeperiod chosen. The overall most laziest wildboar therefore seems to be the wildboar indivual Sabine.
...
The effect of the human influence areas at the activity-patterns of the wild boars is shown in Figure 3. The duration of stay in urban areas is different for each animal. It is clearly seen, that the wild boars "Sabine" and "Ruth" venture closest to settlements. Whereas "Isabelle" and "Rosa" avoid the human influence areas and and prefer to stay in nature. The exact values and the ranking for each wild boar are given in table 2 & 3.

```{r message = FALSE, warning = FALSE, results='hide', fig.show = TRUE, fig.cap = "Figure 3: Time stay in human influence areas and nature as a percentage over the entire study period (31 days) of the seven wild boars."}
...
The preferred locations of wild boars are very different and individual. "Sabine" and "Ruth" for example are water-loving, staying mostly in water bodies and wetlands. Whereas "Caroline" and "Fritz" are more in woods and farmlands. Over all, wetlands are the most popular location for every wild boar. The time stay in the land cover types is shown in figure 4.

```{r message = FALSE, warning = FALSE, results='hide', fig.show = TRUE, fig.cap = "Figure 4: Time stay in the land cover types as a percentage over the entire study period (31 days) of the seven wild boars."}
